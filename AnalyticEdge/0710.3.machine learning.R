...
del <- read.csv("C:/Users/Nas/Documents/R/FlightDelays.csv")
del[1:3,]
...
ttt=table(ynew,gg1)
ttt
#(false positive + false negative) / test size
...
for (i in 2:n2) {
  axis[i]=i
  #ax is expected value of test element result
  ax[i]=xbar*i
  #ay is incremental last value + test element result
  ay[i]=ay[i-1]+bb1[i,2]
}
